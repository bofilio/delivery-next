...
async function fetchFood(filter: any) {
  try {
    const food = (await axios.get(`${BACKEND_URL}/food`, { params: filter }))
      .data;
    return food;
  } catch (err) {
...
export function useOneFood(id: string) {
  return useQuery<foodType, Error>([QueryKeys.food, id], () =>
    fetchFoodById(id)
  );
}
...
export function useFoodByCat(data) {
  return useQuery<foodType[], Error>([QueryKeys.foodByCat, data.id], () =>
    fetchFoodByCat(data.id)
  );
}
async function foodPagination(pageNumber) {

  
  try {
    const food = (
      await axios.get(`${BACKEND_URL}/food?_limit=6&_page=${pageNumber}`)
    ).data;
...
export function useFoodPagination(pageNumber) {
  return useQuery<foodType[], Error>(
    [QueryKeys.food,{pageNumber}],
    () => foodPagination(pageNumber),
    {
