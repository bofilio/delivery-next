...
export default function Messages() {
  ...
  return (
    <div className="flex  flex-grow  items-start w-full space-x-8 mt-12">
      ...
      <div
        className={
          (chatOpen ? "lg:flex hidden" : "flex") +
          " lg:w-1/2 w-full  h-full flex-col lg:pr-6 pr-0 overflow-y-auto "
        }
      >
        ...
        <div className="mt-8 space-y-6 text-xs text-gray-normal">
          <Messager
            onClick={toggleChat}
            tags={[
              { text: "Promotion", color: "primary" },
              { text: "Expire soon", color: "secondary" },
            ]}
            active
          />
          ...
          <Messager
            onClick={toggleChat}
            tags={[{ text: "Promotion", color: "primary" }]}
          />
          <Messager onClick={toggleChat} />
          <Messager onClick={toggleChat} />
          <Messager onClick={toggleChat} />
      ...
      {/**Chat Component */}
      <div
        className={`${chatOpen ? "" : "lg:block hidden"
          } lg:sticky absolute  top-0 right-0 lg:w-1/2 w-full lg:h-168 h-full overflow-y-auto lg:bg-light bg-white p-8 text-gray-normal lg:rounded-xl rounded-none`}
      >
        <button
          className="lg:hidden inline-block p-4 bg-primary rounded-xl  mb-16"
          onClick={toggleChat}
        >
